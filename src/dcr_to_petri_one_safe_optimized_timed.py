        ...
        for event in G['marking']['pending']:
            if event in self.need_pending_place:
                if event in G['marking']['included']:
                    ...
                    pending_place = self.pending_excluded_places[event]
                    if isinstance(pending_place, list):
                        m[pending_place[0]] = 1
                    else:
                        m[pending_place] = 1
            ...
            if event in G['marking']['included'] or event in self.need_included_place:
                if n == 1:
                    ...
                    if event in self.inc_ex_set and len(self.inc_ex_set[event]) > 0:
                        ...
                        # add another type of relation which is equivalent to 1 but also has arcs between pending places
                        for relation in ['excludesTo', 'includesTo']:
                            if event in G[relation]:
                                for event_prime in G[relation][event]:
                                    if event_prime in self.inc_ex_pending_set and not timed_dcr:
                                        inc_ex_permutations.append([(event_prime, 3)])
                    ...
                    if event in self.resp_no_resp_set and len(self.resp_no_resp_set[event]) > 0:
                        ...
                        temp_timed_resp_no_resp_permutations = [list(zip(self.resp_no_resp_set[event], x)) for x in itertools.product(list(range(5, -1, -1)), repeat=len(self.resp_no_resp_set[event]))]
                        rip = []
                        ...
                        for t in ts:
                                ...
                                for event_prime, j in in_ex_perm:
                                    ...
                                    if j == 0: # rule 2
                                        ...
                                        if event in G['excludesTo'] and event_prime in G['excludesTo'][event]:
                                            ...
                                            if event_prime in self.inc_ex_pending_set and not timed_dcr:
                                                inh_arc_place = self.pending_places[event_prime]
                                                inh_arc = pn_utils.add_arc_from_to(inh_arc_place, t, tapn)
                                        ...
                                        if event in G['includesTo'] and event_prime in G['includesTo'][event]:
                                            ...
                                            if event_prime in self.inc_ex_pending_set and not timed_dcr:
                                                inh_arc_place = self.pending_excluded_places[event_prime]
                                                inh_arc = pn_utils.add_arc_from_to(inh_arc_place, t, tapn)
                                                inh_arc.properties['arctype'] = 'inhibitor'
                                    ...
                                    elif j == 3: #this solves the pending excluded mapping
                                        if event in G['includesTo'] and event_prime in G['includesTo'][event]:
                                            pn_utils.add_arc_from_to(self.pending_excluded_places[event_prime],t,tapn)
                                            pn_utils.add_arc_from_to(t,self.pending_places[event_prime],tapn)
                                            pn_utils.add_arc_from_to(t,included_place_event_prime,tapn)
                                            inh_arc = pn_utils.add_arc_from_to(included_place_event_prime,t,tapn)
                                            inh_arc.properties['arctype'] = 'inhibitor'
                                        elif event in G['excludesTo'] and event_prime in G['excludesTo'][event]:
                                            pn_utils.add_arc_from_to(t,self.pending_excluded_places[event_prime],tapn)
                                            pn_utils.add_arc_from_to(self.pending_places[event_prime],t,tapn)
                                            pn_utils.add_arc_from_to(included_place_event_prime,t,tapn)
                            ...
                            if resp_no_resp_perm:
                                for event_prime, r in resp_no_resp_perm:
                                    ...
                                    pe_event_prime = self.pending_excluded_places[event_prime] if event_prime in self.pending_excluded_places else None
                                    ...
                                    if isinstance(pending_place_event_prime, list) and event_prime in self.inc_ex_pending_set:
                                        ...
                                        if r == 0:
                                            ...
                                            # this solves pending excluded places
                                            pn_utils.add_arc_from_to(included_place_event_prime, t, tapn)
                                            if not (event in G['excludesTo'] and event_prime in G['excludesTo'][event]):
                                                pn_utils.add_arc_from_to(t, included_place_event_prime, tapn)
                                        elif r == 1:
                                            ...
                                            pn_utils.add_arc_from_to(pe_event_prime, t, tapn)
                                            inh_arc = pn_utils.add_arc_from_to(included_place_event_prime, t, tapn)
                                            inh_arc.properties['arctype'] = 'inhibitor'
                                            if event in G['responseTo'] and event_prime in G['responseTo'][event]:
                                                pn_utils.add_arc_from_to(t, pe_event_prime, tapn)
                                        elif r == 3:
                                            if event in G['responseTo'] and event_prime in G['responseTo'][event]:
                                                pn_utils.add_arc_from_to(t, pe_event_prime, tapn)
                                                inh_arc0 = pn_utils.add_arc_from_to(pe_event_prime, t, tapn)
                                                inh_arc0.properties['arctype'] = 'inhibitor'
                                                inh_arc = pn_utils.add_arc_from_to(included_place_event_prime, t, tapn)
                                                inh_arc.properties['arctype'] = 'inhibitor'
                            if mi_perm:
